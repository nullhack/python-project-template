...
1. Read `simulation_results.md` (current iteration) and `domain_spec.md` (all contexts). If prior simulation_results exist, read those too for cumulative context.
2. List all unresolved pain points from simulation_results.md across all contexts. These are the fix targets.
3. Rewrite `domain_spec.md` addressing every pain point:
...
8. Update `simulation_results.md` Resolution Status table: mark each pain point as Resolved with a brief description of how. This is the authoritative pain-point tracker — domain_spec does not duplicate them.
9. This is a FULL REWRITE of domain_spec.md, not a patch. Read the entire existing spec (all contexts), then write the updated version from top to bottom. This ensures coherence — no context contradicts another because the whole document was written in one pass with full context.
