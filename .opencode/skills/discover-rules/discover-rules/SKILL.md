---
name: discover-rules
description: "Derive business rules and constraints from domain spec artifacts (events, invariants, commands) and map them to feature files"
---
...
1. Read product_definition.md, domain_spec.md, glossary.md, and all `.feature` files (created by discover-features in this same state) from `in` artifacts.
2. Assign domain spec artifacts to features per [[requirements/rule-derivation#content]]: using the bounded context column in the domain spec's entity table, assign each entity, event, and command to the feature corresponding to its context.
