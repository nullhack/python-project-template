...
8. Create a `.feature` file from the template at `.templates/docs/features/<feature_name>.feature.template` for each feature with title and description. Do NOT write Rules (Business) or Constraints — those come from the discover-rules skill.
9. Run context coverage gap analysis per [[requirements/feature-discovery#content]]: every bounded context covered by at least one feature? IF any gap → flag it in the relevant interview notes.