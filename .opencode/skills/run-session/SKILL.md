...
| Agent | Reads |
|---|---|
| PO (Step 1) | `.flowr/sessions/session.yaml`, `.flowr/flows/feature-flow.yaml`, `scope_journal.md` (resume check), `system.md` (Domain Model section, read-only), `glossary.md`, `docs/post-mortem/` (selective scan), in-progress `.feature` |
| SA (Step 2) | `.flowr/sessions/session.yaml`, `.flowr/flows/feature-flow.yaml`, `system.md`, `glossary.md`, in-progress `.feature`, targeted `.py` files |
| SE (Step 3) | `.flowr/sessions/session.yaml`, `.flowr/flows/feature-flow.yaml`, `system.md`, `glossary.md`, in-progress `.feature`, targeted `.py` files |
| SA (Step 4) | `.flowr/sessions/session.yaml`, `.flowr/flows/feature-flow.yaml`, `system.md`, `glossary.md`, in-progress `.feature`, ADR files referenced in `system.md` |
...
1. Update session file in `.flowr/sessions/`:
...
1. Update the session file in `.flowr/sessions/` to reflect the completed step before doing any other work.
2. Commit the session file update:
   ```bash
   git add .flowr/sessions/session.yaml
...
1. Never skip reading the session file in `.flowr/sessions/` and `.flowr/flows/feature-flow.yaml` at session start
2. Never end a session without updating the session file in `.flowr/sessions/`
