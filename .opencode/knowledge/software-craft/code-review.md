...
- Design correctness is verified before conventions: YAGNI > KISS > DRY > ObjCal > Smells > SOLID > patterns first, then coverage, lint, and type checking.
...
**Design Before Conventions** — Running convention checks on code that might be redesigned is wasted effort — a design change invalidates the convention work. This is why the tiers are ordered: design first, then structure, then conventions. During design review, the reviewer checks design correctness only. During structure review, the reviewer checks test quality and coverage only. Convention concerns are verified last, after design and structure have passed.
