...
- Stubs are created at two levels: project-level creates the package skeleton; feature-level creates typed source stubs and test stubs per feature.
...
- Source stubs are breadcrumbs from the domain model: the SE can add, remove, or modify them as the real shape emerges from TDD.
